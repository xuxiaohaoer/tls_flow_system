...
def pre_data_payload(length, length_1):
    ...
    dataset_train_b = np.load("RF/feature_flow/train_black.npy", allow_pickle=True)
    dataset_train_w = np.load("RF/feature_flow/train_white.npy", allow_pickle=True)
    dataset_test_b = np.load("RF/feature_flow/test_black.npy", allow_pickle=True)
    dataset_test_w = np.load("RF/feature_flow/test_white.npy", allow_pickle=True)
    ...
    for key in dataset:
        if (key[-5] != 0):
            if key[-2] == 'black':
                ...
                label.append(1)
            pay = len_handle(key[-7], length)
            payload.append(find_pay(pay, length_1))
    ratio = len(dataset_train_w) + len(dataset_train_b)
